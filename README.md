...
### ! На хостовой ОС настроить тип  сетевой мост с ВМ.
Запросы к GitLab будут идти через внешний NGINX на ВМ и проксироваться в GitLab Docker контенйнер на 8443 порт.
 - Настроить:
```sudo nano /etc/nginx/sites-available/gitlab```
- включить:
```sudo ln -s /etc/nginx/sites-available/gitlab /etc/nginx/sites-enabled/```
#### Конфиг взять в nginx/gitlab
#### перезагрузи ``` sudo systemctl reload nginx```
